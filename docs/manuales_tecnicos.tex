...
A continuación, se detalla la topología del repositorio, donde el directorio raíz se ha simplificado para contener únicamente la documentación principal y el documento de la memoria, organizando todo el código fuente dentro del directorio \texttt{src/}:

\begin{itemize}
    \item \textbf{Directorio Raíz}:
    \begin{itemize}
        \item \texttt{README.md}: Documento de presentación y guía rápida del repositorio.
        \item \texttt{tfg.pdf}: Documento completo de la memoria académica oficial en formato PDF.
        \item \texttt{.gitignore}: Filtro de exclusión para archivos temporales y binarios del sistema.
    \end{itemize}

    \item \textbf{Módulo de Código Fuente (\texttt{src/})}: Directorio que agrupa toda la lógica ejecutable del pipeline:
    \begin{itemize}
        ...
        \item \texttt{train.ipynb}: Jupyter notebook para pruebas rápidas de entrenamiento.
        \item \textbf{\texttt{data\_processors/}}: Subdirectorio con los scripts de carga e inicialización de logs de eventos.
    \end{itemize}

    \item \textbf{Orquestación HPC (\texttt{slurm/})}: Colección de scripts \textit{bash} para el gestor Slurm en el clúster FinisTerrae III del CESGA.
    \begin{itemize}
    ...
    \item \textbf{Datos de Entrada (\texttt{data/})}: Almacenamiento de matrices y datos de entrenamiento.
    \begin{itemize}
        ...
        \item \textbf{\texttt{event\_log/}}: Carpeta que almacena las trazas crudas y particionadas en XES de \textit{env\_permit}.
    \end{itemize}
    
    \item \textbf{Resultados Consolidados (\texttt{resultados/})}: Registro de telemetría final consolidado y figuras en alta resolución.
    \begin{itemize}
        \item \texttt{resultados\_soluciones\_reales.csv}: Base de datos unificada de las 55 soluciones óptimas.
        \item Archivos CSV individuales por solver y figuras de visualización comparativa (Pareto, factibilidad, etc.).
    \end{itemize}

    \item \textbf{Documentación (\texttt{docs/})}:
    \begin{itemize}
        \item Ficheros LaTeX y borradores Markdown de soporte metodológico.
    \end{itemize}
\end{itemize}
