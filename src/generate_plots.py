...
def main():
    ...
    # --- PLOT 1: EXTREME TIME OPPOSITION (QPU VS SIMULATION) ---
    ...
    categories = [
        'Opt. Bayesiana (Clásica)', 
        'VQE (Simulación Ideal FT3)', 
        'VQE (QPU Qmio Real)', 
        'VarQITE (Simulación Ideal FT3)', 
        'VarQITE (QPU Qmio Real)'
    ]
    times = [bayes_t, vqe_ideal_t, vqe_qpu_t, qite_ideal_t, qite_qpu_t]
    colors = ['#2ca02c', '#aec7e8', '#1f77b4', '#ffbb78', '#ff7f0e']

    bars = plt.bar(categories, times, color=colors, edgecolor='black', width=0.6, zorder=3)
    plt.yscale('log')
    plt.ylabel('Tiempo de Ejecución (Segundos - Escala Log)', fontsize=12, fontweight='bold')
    plt.title('Paradoja de Tiempos de Ejecución: QPU Física Real vs. Simulación vs. Clásico', fontsize=14, fontweight='bold', pad=15)
    plt.xticks(rotation=20, ha='right', fontsize=10, fontweight='bold')
    
    for bar in bars:
        height = bar.get_height()
        plt.text(bar.get_x() + bar.get_width()/2.0, height * 1.1, f'{height:.1f} s', ha='center', va='bottom', fontsize=10, fontweight='bold')
    ...
    categories = ['Opt. Bayesiana (Clásica)', 'VQE (QPU Real Qmio)', 'VarQITE (QPU Real Qmio)']
    energies = [bayes_e, vqe_qpu_e, qite_qpu_e]
    colors = ['#2ca02c', '#d62728', '#ff7f0e']

    bars = plt.bar(categories, energies, color=colors, edgecolor='black', width=0.5, zorder=3)
    ...
    categories = ['Opt. Bayesiana', 'VQE (QPU Real)', 'VarQITE (QPU Real)']
    rates = [bayes_f, vqe_qpu_f, qite_qpu_f]
    colors = ['#2ca02c', '#d62728', '#ff7f0e']

    bars = plt.bar(categories, rates, color=colors, edgecolor='black', width=0.5, zorder=3)
    ...
    categories = ['VQE (Sim. Ideal)', 'VQE (QPU Real)', 'VarQITE (Sim. Ideal)', 'VarQITE (QPU Real)']
    violations = [vqe_ideal_v, vqe_qpu_v, qite_ideal_v, qite_qpu_v]
    colors = ['#aec7e8', '#1f77b4', '#ffbb78', '#ff7f0e']

    bars = plt.bar(categories, violations, color=colors, edgecolor='black', width=0.6, zorder=3)
    ...
    print("All 8 plots generated successfully in: resultados/")
